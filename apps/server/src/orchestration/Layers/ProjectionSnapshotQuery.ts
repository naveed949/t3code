...
function mapLatestTurn(
  ...
  return {
    ...
    ...(row.sourceProposedPlanThreadId !== null && row.sourceProposedPlanId !== null
      ...
      : {}),
    ...(row.skillInvocation !== null ? { skillInvocation: row.skillInvocation } : {}),
  };
}
...
function mapWorkflowAttachmentState(row: Schema.Schema.Type<typeof ProjectionThreadDbRowSchema>) {
  const state = row.workflowAttachmentState;
  if (state == null) return {};
  return {
    ...(state.hint !== undefined ? { workflowAttachmentHint: state.hint } : {}),
    ...(state.attachment !== undefined ? { workflowAttachment: state.attachment } : {}),
  };
}
...
const makeProjectionSnapshotQuery = Effect.gen(function* () {
          ...
          ([
            ...
            Effect.gen(function* () {
              ...
              const threads: ReadonlyArray<OrchestrationThread> = threadRows.map((row) => ({
                ...
                ...mapWorkflowAttachmentState(row),
                deletedAt: row.deletedAt,
  ...
  const getThreadShellById: ProjectionSnapshotQueryShape["getThreadShellById"] = (threadId) =>
    Effect.gen(function* () {
      ...
      return Option.some({
        ...
        ...mapWorkflowAttachmentState(threadRow.value),
  ...
  const getSkillRunsByThreadId: ProjectionSnapshotQueryShape["getSkillRunsByThreadId"] = Effect.fn(
    "ProjectionSnapshotQuery.getSkillRunsByThreadId",
  )(
    function* (threadId) {
      const rows = yield* listSkillRunRowsByThreadId({ threadId });
      return rows.map((row) => row.skillInvocation);
    },
    (effect) =>
      effect.pipe(
        Effect.mapError(
